...
def animation(variable_sequence, disturbances_dict, save=False): #Animation of just u
    ...
    time_skip = 5 #intervals between displayed frame
    ...
    anim = animator.FuncAnimation(
        fig, lilly, range(len(variable_sequence) // time_skip), interval=5 #display duration for each frame in milliseconds
    )
    plt.show()
    # if(save):
