...
def compute(timesteps = default_timesteps, disturbance_count = 0, max_wave_count = 1, wave_thickness = 1, 
            initial_condition_type: str = "center", laplacian_method = laplacian_standard, 
            save: bool = False, u_filename = "", v_filename = "") -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    while counter <= disturbance_count:
        index = np.random.randint(0,timesteps)
    ...
    return u_sequence, v_sequence, disturbances_dict

def generate_random_sequences(sequence_count: int = 10, timesteps = 3000, disturbance_count=5, max_wave_count=2, wave_thickness=1,
                                initial_condition_type: str ="center", laplacian_method=laplacian_standard):
    ...
    for i in range(sequence_count):
        print(f"\nSimulation {i+1}.")
        u_sequences[i], v_sequences[i], disturbances_dict = compute(timesteps = timesteps, disturbance_count = disturbance_count, max_wave_count = max_wave_count, wave_thickness = wave_thickness,
...
def animation(u_sequence, v_sequence, disturbances_dict, save=False): #Animation of just u
    """NOTE: this function was modified from code mentor @ UCSB wrote. (all other functions were written originally)"""
    u_sequence = u_sequence[:,1:x_grid_size-2,1:y_grid_size-2] #Filter out borders, which are affected by Neumann conditions
    ...
    img_u = ax_1.imshow(u_sequence[0], cmap="Blues", interpolation="none", vmin=0, vmax=1)
    #img_v = ax_2.imshow(v_sequence[0], cmap="Greys", interpolation="none", vmin=0, vmax=1)


    def lilly(t):
        current_time = t * time_skip
        img_u.set_data((u_sequence[current_time]))
    ...
    anim = animator.FuncAnimation(
        fig, lilly, range(len(u_sequence) // time_skip), interval=10 #display duration for each frame in milliseconds
    )
    plt.show()
    # if(save):
...
# u_sequence, v_sequence, disturbances_dict = compute(timesteps = 3000, disturbance_count=3, max_wave_count=2, wave_thickness=1,
#                                 initial_condition_type="center", laplacian_method=laplacian_standard, save=False, 
...
sequence_count = 2
u_sequences, v_sequences, disturbances_dict_list = generate_random_sequences(sequence_count=sequence_count, timesteps=2000)
